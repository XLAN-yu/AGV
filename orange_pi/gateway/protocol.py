...
VERSION = 1
MAX_PAYLOAD = 256
STATUS_FLAG_BLE_PAIR_REQUEST = 0x01
STATUS_FLAG_OBSTACLE_GUARD_ENABLED = 0x02
STATUS_FLAG_OBSTACLE_GUARD_CAPABLE = 0x04
STATUS_FLAGS_KNOWN = (
...
_DRIVE_HOLONOMIC = struct.Struct("<fff")
_WHEEL_TEST = struct.Struct("<Bh")
...
class MessageType(IntEnum):
    ...
    SET_OBSTACLE_GUARD = 0x05
    ORANGE_PI_SHUTDOWN_REQUEST = 0x06
    WHEEL_TEST = 0x07
    STATUS = 0x80
    ACK = 0x81
...
def encode_holonomic_drive_payload(
    linear_mps: float, lateral_mps: float, angular_rps: float
) -> bytes:
    if not all(math.isfinite(value) for value in (linear_mps, lateral_mps, angular_rps)):
        raise ValueError("drive values must be finite")
    return _DRIVE_HOLONOMIC.pack(linear_mps, lateral_mps, angular_rps)


def encode_obstacle_guard_payload(enabled: bool) -> bytes:
    if not isinstance(enabled, bool):
        raise ValueError("obstacle guard value must be boolean")
    return bytes((1 if enabled else 0,))


def encode_wheel_test_payload(wheel_index: int, target_rpm: int) -> bytes:
    if wheel_index not in range(4) or target_rpm == 0 or abs(target_rpm) > 40:
        raise ValueError("wheel test requires wheel 0..3 and RPM -40..40 excluding zero")
    return _WHEEL_TEST.pack(wheel_index, target_rpm)
