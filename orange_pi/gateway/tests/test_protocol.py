...
def test_holonomic_drive_payload_uses_three_little_endian_floats() -> None:
    payload = encode_holonomic_drive_payload(0.25, -0.3, 1.2)
    assert len(payload) == 12
    assert struct.unpack("<fff", payload) == pytest.approx((0.25, -0.3, 1.2))


def test_wheel_test_payload_is_bounded_and_little_endian() -> None:
    assert encode_wheel_test_payload(2, -30) == struct.pack("<Bh", 2, -30)
    with pytest.raises(ValueError):
        encode_wheel_test_payload(4, 30)
    with pytest.raises(ValueError):
        encode_wheel_test_payload(0, 41)
